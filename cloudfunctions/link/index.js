...
exports.main = async(event, context) => {
  ...
  } else if (event.NODEJS == "editjielongtask") {
    return await editjielongtask(event);
  ...
  } else if (event.NODEJS == "quitmeeting") {
    return await quitmeeting(event);
  } else if (event.NODEJS == "delmeeting") {
    return await delmeeting(event);
  ...
  } else if (event.NODEJS == "sendyijian") {
    return await sendyijian(event);
  }
}
...
async function editjielongtask(event) {
  const wxContext = cloud.getWXContext()
...
async function quitmeeting(event) {
...
async function delmeeting(event) {
  await db.collection('link_user').where({
    taskid: event.taskid,
  }).remove()
  return await db.collection('link_task').where({
    taskid: event.taskid,
...
async function sendyijian(event) {
  const wxContext = cloud.getWXContext()
